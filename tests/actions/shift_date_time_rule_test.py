...
def test_shift_date_time_rule_simple_two_rules_one_day(one_task_state: State):
    batching_rule = TimetableGenerator.batching_size_rule(TimetableGenerator.FIRST_ACTIVITY, 1)
    ...
    state = one_task_state.replace_timetable(
        batch_processing=[
            TimetableGenerator.batching_size_rule(TimetableGenerator.SECOND_ACTIVITY, 3),
            batching_rule,
    ...
    assert new_state.timetable.batch_processing[1].firing_rules[2][0].value == DAY.MONDAY
    ...
    assert new_state.timetable.batch_processing[1].firing_rules[1][0].value == DAY.MONDAY
