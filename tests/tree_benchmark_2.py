...
class Heap:
    ...
    def add_points(self, points, query_point):
        for point in points:
            distance = euclidean_distance(point, query_point)
            heapq.heappush(self.heap, (distance, tuple(point)))

    def refresh_distances(self, query_point):
        # Rebuild the heap with updated distances
        new_heap = []
        while self.heap:
            dist, point = heapq.heappop(self.heap)
            new_distance = euclidean_distance(point, query_point)
            heapq.heappush(new_heap, (new_distance, point))
        self.heap = new_heap
...
def benchmark():
    ...
    for iteration in range(num_iterations):
        ...
        heap.add_points(new_points, query_point)
        heap.refresh_distances(query_point)
        for _ in range(pops_per_iteration):
            nearest = heap.pop_nearest()
    ...
    # Average times
    avg_time_heap = sum(times_heap) / len(times_heap)
